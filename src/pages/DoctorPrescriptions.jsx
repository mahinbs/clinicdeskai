...
const DoctorPrescriptions = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] relative">
      {showMobileQueue && (
        <div className="fixed inset-0 bg-black/20 z-40 md:hidden backdrop-blur-sm" onClick={() => setShowMobileQueue(false)} />
      )}

      <div className={`absolute md:relative inset-y-0 left-0 z-50 bg-white w-3/4 md:w-72 border-r border-gray-200 transform transition-transform duration-300 ease-in-out ${showMobileQueue ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 flex flex-col`}>
        ...
        <div className="p-4 md:p-0 md:pr-6 md:pl-2 overflow-y-auto flex-1">
          <h2 className="hidden md:block text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 px-2 mt-4">Patients</h2>
          {loading ? (
            <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-teal-600 mx-auto my-4" />
          ) : (
            <div className="space-y-2">
              {patients.slice(0, 10).map((p) => (
                <button
                  key={p.id}
                  onClick={() => { setSelectedPatient(p); setShowMobileQueue(false); }}
                  className={`w-full cursor-pointer text-left p-3 rounded-xl transition-all duration-200 group relative overflow-hidden ${current?.id === p.id ? 'bg-teal-50 ring-1 ring-teal-200 shadow-sm' : 'hover:bg-gray-50'}`}
                >
                  <div className="relative z-10">
                    <p className={`font-bold text-sm ${current?.id === p.id ? 'text-teal-900' : 'text-gray-900'}`}>{p.full_name}</p>
                    <p className={`text-xs mt-1 ${current?.id === p.id ? 'text-teal-600' : 'text-gray-500'}`}>{p.gender || '—'}, {p.date_of_birth ? new Date().getFullYear() - new Date(p.date_of_birth).getFullYear() : '—'} years</p>
                  </div>
                  {current?.id === p.id && (
              ...
              {patients.length === 0 && <p className="text-sm text-gray-500 p-2">No patients.</p>}
            </div>
          )}
        </div>
      </div>
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <button onClick={() => setShowMobileQueue(true)} className="md:hidden p-2 -ml-2 hover:bg-gray-100 rounded-lg text-gray-600">
                      <Menu className="w-6 h-6" />
                    </button>
                    <h1 className="text-xl md:text-2xl font-bold text-gray-900 truncate">Rx: {current.full_name}</h1>
                  ...
                  <p className="text-sm text-gray-500 ml-1 md:ml-0">{current.patient_id_number} • {current.date_of_birth ? new Date().getFullYear() - new Date(current.date_of_birth).getFullYear() : '—'}yrs • {current.gender || '—'}</p>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                ...
                <Card className="lg:col-span-2 space-y-6">
                  <div>
                    ...
                    <textarea className="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white shadow-sm focus:border-teal-500 focus:ring-teal-500/20 transition-all p-4 text-gray-900 min-h-[80px]" placeholder="Enter primary diagnosis..." />
                  </div>
                  <div className="pt-2">
                    ...
                    <div className="border border-gray-200 rounded-xl bg-white shadow-inner flex flex-col h-[400px]">
                      ...
                      <div className="p-3 bg-gray-50 border-t border-gray-100 text-right shrink-0">
                        <p className="text-xs font-medium text-teal-700">Signed electronically by {profile?.full_name || 'Doctor'}</p>
                      </div>
                    </div>
                  </div>
                </Card>
