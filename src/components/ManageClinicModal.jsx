...
import { MapPin, Copy, CheckCircle, AlertCircle, UserX, UserCheck, KeyRound, X } from 'lucide-react';
...
export default function ManageClinicModal({ clinicId, open, onClose, onUpdated }) {
  ...
  return (
    <Modal open={true} onClose={onClose} contentClassName="max-w-4xl max-h-[90vh] overflow-y-auto">
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Manage clinic</h2>
          {clinic && (
            <>
              <p className="text-lg font-semibold text-gray-800 mt-1">{clinic.name}</p>
              <p className="text-sm text-gray-500 flex items-center gap-1 mt-0.5">
                {clinic.address ? <><MapPin className="w-4 h-4" /> {clinic.address}</> : 'No address'}
              </p>
            </>
          )}
        </div>
      ...
      {loading && !clinic ? (
        ...
        <p className="text-gray-500 py-4">Clinic not found.</p>
      ) : (
                      ...
                      <tr key={u.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => handleResetPassword(u)} disabled={!!actionLoading} title="Reset password">
